Now that the configuration includes our unseal key, we can deploy OpenBao using Helm. The repository has already been set up for you so you can directly use `helm install openbao openbao/openbao -f /root/assets/chart-values.yaml --version 0.25.6`
...
(Please wait until the pod reports status as "Running" like below, to continue.)
```
