...
* `bao login` command : This command will prompt the user for the token. It also obfuscate the token and it will not be leaked to shell history.
...
* `BAO_TOKEN` environment variable : Many tools support this method, however keep in mind that the token might get leaked to the shell history. We don't recommend using this if alternative are available.
...
A single rule is formed by the [API path](https://openbao.org/api-docs/) and capabilities granted to that path. OpenBao policies are deny by default thus if nothing specified access will be denied. We will go over policies in another labs.

Now that we are logged, we can try several things aside from reading policies.
For example : 
* We can check which auth methods are enabled : `bao auth list`
* We can check which secrets engine are enabled : `bao secrets list`
* We can list all the policies : `bao policy list`
* We can list all the nodes in the cluster : `bao operator members list`
* Feel free to also try out more stuff by [checking the documentation](https://openbao.org/docs/commands/))

You can also explore the API, using verbs (read, list, write, delete). So for example to list the policies, instead of `bao policy list` you can do 
